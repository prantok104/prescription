...
function getTheInsertMedecine($getpid){
    ...
    while($row = $result->fetch_array(MYSQLI_ASSOC)){
        echo '<li id="'.$row['id'].'">
                    ...
                    <span>'.$row['orders'].'</span> 
                    <strong class="text-primary pull-right">নির্দেশমত: '.$row['time'].'</strong> ';?>
<a href="functions.php?ebid=<?php echo $row['id'];?>&fid=<?php  echo $getfid;?>&pid=<?php echo $getpid;?>" class="btn btn-xs">&times;</a>
<?php 
        echo ' </li> ';
    }
}
...
?>
<b id="up" class="alert alert-success"></b>
    ...
    <div class="prescribe-area">
        <div class="pre-headar">
            <div class="pre-dif text-left">
               <input type="hidden" id="pid" value="<?php  ?>">
               <input type="hidden" id="illid" value="<?php  ?>">
                <strong>ID: </strong> <span><?php echo substr($row['fevername'],0,3).$getpid;?></span>
                <strong>Name: <input type="text" name="pname" id="pname" value="<?php echo $row['pname'];?>"></strong> <span></span>
            </div>
            <div class="pre-dif text-right">
                <strong>Yrs: </strong> <span><input type="text" name="yrs" id="yrs" value="<?php echo $row['age'];?>"></span>
    ...
    <div class="preview-area">
        ...
        <div class="pres-preview">
            ...
            <div class="prescribe">
                ...
                <a href="" class="btn btn-sm bg-primary" data-target="#exampleModalLong3" data-toggle="modal">print</a>
            </div>
        </div>
    </div>
    <!--Prescribe Preview area start-->
            ...
            <div class="modal-body">
                <div class="modal-form custom-body">
                    <form action="" method="POST">
                        <input type="hidden" name="patintid" value="<?php  ?>">
                        <label for="icon"><img src="../assets/img/delta.png" alt="" style="width:30px; height:30px"><input type="radio" name="icon" id="icon" value="1"></label>
                        <label for="icon2"><img src="../assets/img/rx.png" alt="" style="width:30px; height:30px"><input type="radio" name="icon" id="icon2" value="2"></label> 
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="modal-form custom-body">
                    <form action="" method="POST">
                        <div class="times">
                            <input type="hidden" name="pid" id="pids" value="<?php echo $getpid;?>">
...
<!--Patient name and age update-->
<script>
    $(document).ready(function(){
        $('#pname,#yrs').keyup(function(){
            var puname = $('#pname').val();
            var pid = $('#pids').val();
            var yrs = $('#yrs').val();
            if(puname!='' && yrs !=''){
                $.ajax({
                    url: 'functions.php',
                    method:'POST',
                    data:{puname:puname, pid:pid , yrs:yrs},
                    success:function(data){
                        $('#up').html(data);
                    }
                });
        ...
        $('#page').keyup(function(){
            ...
            if(page!=''){
                $.ajax({
                    url: 'functions.php',
                    method:'POST',
                    data:{page:page, pid:pid , illid:illid},
                    success:function(data){
                        $('#up').html(data);
                    }
                });
